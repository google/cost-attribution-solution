...
module "storage" {
  ...
  region           = var.region
  bucket_name      = var.bucket_name
  # object_name      = "function_source.zip"      # ✅ Ensure this is set
  # source_file_path = "./function_source.zip"    # ✅ Ensure this matches the actual ZIP path
}
...
module "cloud_function" {
  ...
  function_bucket   = module.storage.bucket_name
  pubsub_topic_id   = module.pubsub.vertex_ai_audit_topic_id
  object_name       = module.storage.object_name  # ✅ Correct output reference
  depends_on = [
